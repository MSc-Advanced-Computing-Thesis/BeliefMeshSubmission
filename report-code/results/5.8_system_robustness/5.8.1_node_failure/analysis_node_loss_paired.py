...
from _shared.averaged_readout import load_run, validate_reduction

NEW = Path(ART + "/5.8_system_robustness/5.8.1_node_failure/node_loss")
# the earlier argmax-only runs this analysis originally cross-checked against
# are not part of the repository; the guard that uses them is skipped when absent
OLD = Path(ART + "/5.8_system_robustness/5.8.1_node_failure/node_loss_original_argmax_only")
POST_WINDOW = (340, 390)          # verbatim from run_node_failure.py
OUT_CSV = Path(ART + "/5.8_system_robustness/5.8.1_node_failure/node_loss_paired_averaged.csv")
...
if len(go):
    ...
    print("GUARD 2 skipped: the original argmax-only runs are not part of this repository")
