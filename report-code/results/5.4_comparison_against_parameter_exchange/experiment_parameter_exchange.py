...
import experiment_gossip_comparator as GC
...
DEFAULT_ROOT = ART + "/5.4_comparison_against_parameter_exchange/five_environments"
# the already-reported single-field runs, used only by --verify-inert
STORED = Path(ART + "/5.4_comparison_against_parameter_exchange/five_environments/field0_original/gossip_cmp_fusion")
...
if __name__ == "__main__":
    ...
    a = ap.parse_args()
    if a.verify_inert:
        sys.exit(0 if verify_inert(_SCRATCH / "gossip_inert") else 1)
    main(a.root)
