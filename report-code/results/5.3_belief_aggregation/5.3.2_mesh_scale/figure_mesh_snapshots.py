...
from _shared.averaged_readout import averaged_params, half_width
from _shared.artefacts import load_array
from beliefmesh.simulation.offset_fields import build_dynamic_offset_field

RUN = Path(ART + "/5.3_belief_aggregation/5.3.2_mesh_scale/snapshot_run/fusion_random_seed42")
FIGS = FIG_DIR
NAME = "fig_5_6_mesh_snapshots"
WIDTH, DPI = 6.3, 200
LAST = 50
...
def load():
    bel = load_array(RUN, "cell_beliefs_steps").astype(np.float64)
    nc = load_array(RUN, "cell_ncov_steps").astype(int)
    paths = load_array(RUN, "realised_wearable_paths")   # (W, T, 2) row, col
