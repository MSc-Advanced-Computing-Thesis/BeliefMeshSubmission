...
RUNS = Path(ART + "/5.1_baseline_model_and_calibration")
FIGS = FIG_DIR
WIDTH = 6.3
...
def fig1_error_distribution():
    d = read_dump(RUNS / "r1a_held_out_evaluation" / "per_sample_baseline.csv")
    dec = load_csv(RUNS / "r1a_held_out_evaluation" / "rms_by_uncertainty_decile.csv")
...
def fig3_colour_sweep():
    s = load_csv(RUNS / "r1b_colour_sweep" / "sweep.csv")
    ...
    fig.tight_layout()
    save(fig, "fig_5_3a_colour_sweep")


def fig4_offset_sweep():
    s = load_csv(RUNS / "r1c_offset_sweep" / "sweep.csv")
    ...
    for ax in axes:
        ...
        ax.yaxis.label.set_size(8)
    fig.tight_layout()
    save(fig, "fig_5_3b_offset_sweep")
