...
class Mesh:
    ...
    def __init__(self, node_centres: np.ndarray, fov_size: int, grid_size: int,
                 environment: GridEnvironment, pretrained_path: str | Path | dict[str, str | Path] | list,
        ...
        for i, (cx, cy) in enumerate(node_centres):
            ...
            if isinstance(pretrained_path, dict):
                ckpt = pretrained_path[variant]
            elif isinstance(pretrained_path, (list, tuple)):
                ckpt = pretrained_path[i]          # one checkpoint per node, by index
            else:
