...
def run_one(cfg, uniform, centres, field, paths, tag, rho):
...
def main(part: str, shard: int = 0, nshards: int = 1):
    cfg, uniform, centres, field, paths = _setup()

    if part == "baseline":
        run_one(cfg, uniform, centres, field, paths, "consensus_baseline_rho0p2", 0.2)
        ...
        results = []
        for rho in my_rhos:
            tag = f"consensus_rho{str(rho).replace('.', 'p')}"
            whole_run, r = run_one(cfg, uniform, centres, field, paths, tag, rho)
        ...
        for rho, whole_run, r in results:
            print(f"rho={rho:<5} whole_run={whole_run:.4f} r={r:.4f}")
        print("=== DONE ===")
    else:
